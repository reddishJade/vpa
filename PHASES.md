...
## Phase 3: ISA Translation Agent + Per-Commit Ledger
...
Add a new operation type to `RepairEngine.agent_loop`:

```python
def isa_translate(self, commit_context, analysis, gate_decision, local_repo) -> AgentLoopResult:
    context = build_semantic_port_context(...)
    return self._run_tool_loop(
        op="translate",
        tools=[read, apply_patch, write, grep],
        system_prompt="You are VPA's ISA translation engine...",
        user_content={"reference_patches": ..., "target_files": ...},
        max_retries=3,
    )
```
...
```json
{
  "changes": [
    {
      "op": "modify",
      "path": "src/dynarec/sw64_core3/emit_flags.c",
      "edits": [
        {"old": "static void emit_flags_update(...) {\n    ...\n}",
         "new": "static void emit_flags_update(...) {\n    ...\n}"}
      ]
    },
    {
      "op": "create",
      "path": "src/dynarec/sw64_core3/new_file.c",
      "content": "// Auto-generated by VPA\n..."
    }
  ]
}
```
...
The harness:
1. Reads each target file, records `before_sha256`
2. For each `edit`, searches for exact `old` match (one hit required)
3. Applies `new` replacement in memory
4. Verifies no conflict markers in the result
5. Computes `after_sha256`
6. Writes to disk via temp file + atomic rename
...
def _execute_isa_translate(self, planned: PlannedCommit) -> ExecutedCommit:
    ...
    result = self.repair_engine.isa_translate(
        planned.context, planned.analysis, planned.gate_decision, self.config.local_repo,
    )
    if result.failure_code is not None:
        self.local_git.reset_to_checkpoint(checkpoint)
        return ExecutedCommit(
            planned=planned, method=PromotionMethod.SEMANTIC_PORT,
            apply_status="exhausted" if result.failure_code != FailureCode.NO_LLM_CONFIGURED else "not_run",
            apply_reason=result.status_reason,
            ...
        )
    validation = run_validation(...)
    return ExecutedCommit(
        planned=planned, method=PromotionMethod.SEMANTIC_PORT,
        apply_status="committed", integrity_status="passed",
...
The `LedgerRecord` already supports three-axis state from P1.6. Wire it into
`execute()`:
...
lines.append(f"  apply: {item.apply_status}")
if item.apply_reason:
    lines.append(f"  reason: {item.apply_reason}")
lines.append(f"  integrity: {item.integrity_status}")
